...
export default class InventoryPresenter {
  ...
  async loadItems() {
    try { this.view.showLoading(true); const response = await api.get(this.apiUrl); this.view.setItems(response.data.data); } 
  ...
  async uploadStock(type, dataArray) {
    try {
      ...
      this.view.setUploadTotal(dataToList.length); this.view.setUploadCount(0);
      let successCount = 0, errorCount = 0;
      for (const item of dataToList) {
        try { await InventoryModel.uploadStock(type, item); successCount++; this.view.setUploadCount(successCount); this.view.updateProgress((successCount / dataToList.length) * 100); } 
  ...
  async getHistory(code) {
    try { this.view.showLoading(true); const response = await api.get(`${this.apiUrl}/history/${code}`); this.view.setHistoryData(response.data.data); } 
    catch (error) { this.view.showError('Gagal mengambil riwayat.'); } 
    finally { this.view.showLoading(false); }
  }

  async editTransaction(id, data) {
    try { this.view.showLoading(true); await api.put(`${this.apiUrl}/transaction/${id}`, data); this.view.onUpdateSuccess('Riwayat berhasil diedit & Stok disesuaikan'); this.loadItems(); } 
  ...
  async updateItem(id, itemData) {
    ...
    try { this.view.showLoading(true); await api.put(`${this.apiUrl}/${id}`, { name: itemData.name, stock: parseInt(itemData.stock || 0), shopName: itemData.shopName }); this.view.onUpdateSuccess('Barang berhasil diperbarui'); this.loadItems(); } 
    catch (error) { this.view.showError(error.response?.data?.message || 'Gagal mengupdate barang.'); } 
    finally { this.view.showLoading(false); }
  }

  async deleteItem(id) {
    try { this.view.showLoading(true); await api.delete(`${this.apiUrl}/${id}`); this.view.onDeleteSuccess('Barang berhasil dihapus'); this.loadItems(); } 
    catch (error) { this.view.showError(error.response?.data?.message || 'Gagal menghapus barang.'); } 
    finally { this.view.showLoading(false); }
  }

  async bulkDelete(ids) {
    try { this.view.showLoading(true); await InventoryModel.bulkDelete(ids); this.view.onDeleteSuccess(`${ids.length} barang beserta riwayatnya berhasil dihapus`); this.loadItems(); } 
    catch (error) { this.view.showError(error.response?.data?.message || 'Gagal menghapus barang.'); } 
    finally { this.view.showLoading(false); }
  }
