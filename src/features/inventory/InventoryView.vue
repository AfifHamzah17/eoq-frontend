...
<button @click="exportToExcel" title="Export Excel" class="flex items-center gap-1.5 px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition text-xs font-medium shadow-sm whitespace-nowrap"><i class="fa-solid fa-file-arrow-down"></i> Export</button>
<button @click="openUploadModal('in')" title="Upload Barang Masuk" class="flex items-center gap-1.5 px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition text-xs font-medium shadow-sm whitespace-nowrap"><i class="fa-solid fa-arrow-up-from-bracket"></i> Barang Masuk</button>
<button @click="openUploadModal('out')" title="Upload Barang Keluar" class="flex items-center gap-1.5 px-3 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg transition text-xs font-medium shadow-sm whitespace-nowrap"><i class="fa-solid fa-arrow-down-to-bracket"></i> Barang Keluar</button>
</div>
</div>
</div>
...
<div class="flex items-center gap-2 text-sm"><span class="text-gray-600">Tampilkan:</span><select v-model="perPageSelect" @change="itemsPerPage = perPageSelect === 'custom' ? (customVal||10) : Number(perPageSelect); isCustom = perPageSelect === 'custom';" style="border:1px solid #d1d5db;border-radius:0.5rem;padding:6px 8px;font-size:0.875rem;outline:none;cursor:pointer;"><option value="5">5</option><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="custom">Custom...</option></select><input v-if="isCustom" type="number" v-model.number="customVal" min="1" @input="if(customVal>=1) itemsPerPage=customVal; else if(!customVal) itemsPerPage=1;" placeholder="Angka" style="border:1px solid #d1d5db;border-radius:0.5rem;padding:6px 8px;font-size:0.875rem;outline:none;width:70px;-moz-appearance:textfield;"></div>
...
<Teleport to="body">
<Transition name="fab">
<div v-if="selectedItems.size >= 2" @click="isBulkDeleteModalOpen = true" class="fixed bottom-6 right-6 z-[9999] bg-red-600 text-white px-5 py-3 rounded-full shadow-2xl flex items-center gap-2 cursor-pointer hover:bg-red-700 transition-all"><i class="fa-solid fa-trash"></i><span class="font-semibold text-sm">Hapus ({{ selectedItems.size }})</span></div>
</Transition>
</Teleport>
<Teleport to="#modal-portal"><Transition name="modal"><div v-if="isUploadModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4"><div @click="closeUploadModal" class="absolute inset-0 bg-black/60 backdrop-blur-sm"></div><div class="relative bg-white w-full max-w-3xl rounded-2xl shadow-2xl border border-gray-100 flex flex-col z-10 max-h-[90vh] pointer-events-auto"><div class="px-6 py-4 border-b flex justify-between items-center" :class="transactionType === 'in' ? 'bg-emerald-50' : 'bg-orange-50'"><h3 class="text-lg font-bold text-gray-800"><i :class="transactionType === 'in' ? 'fa-solid fa-truck-ramp-box text-emerald-600' : 'fa-solid fa-truck-ramp-down text-orange-600'" class="mr-2"></i> Upload {{ transactionType === 'in' ? 'Barang Masuk' : 'Barang Keluar' }}</h3><button type="button" @click="closeUploadModal" class="text-gray-400 hover:text-red-500 transition"><i class="fa-solid fa-xmark text-xl"></i></button></div><div class="flex border-b"><button type="button" @click="uploadTab = 'single'" :class="['flex-1 py-3 text-sm font-medium transition-colors focus:outline-none', uploadTab === 'single' ? 'border-b-2 border-blue-600 text-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-700']">Input Satuan</button><button type="button" @click="uploadTab = 'excel'" :class="['flex-1 py-3 text-sm font-medium transition-colors focus:outline-none', uploadTab === 'excel' ? 'border-b-2 border-blue-600 text-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-700']">Upload Excel</button></div><div class="p-6 overflow-y-auto"><form v-if="uploadTab === 'single'" @submit.prevent="handleSingleUpload" class="space-y-4"><div class="grid grid-cols-2 gap-4"><div><label class="block text-sm font-medium text-gray-700 mb-1">Tanggal</label><input v-model="uploadForm.date" type="date" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" required /></div><div><label class="block text-sm font-medium text-gray-700 mb-1">Kategori (Asal Toko)</label><input v-model="uploadForm.shopName" type="text" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="Contoh: Toko Maju" required /></div><div class="col-span-2"><label class="block text-sm font-medium text-gray-700 mb-1">Nama Barang</label><input v-model="uploadForm.itemName" type="text" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="Contoh: Parfum Mawar" required /></div><div class="col-span-2"><label class="block text-sm font-medium text-gray-700 mb-1">Jumlah Barang</label><input v-model.number="uploadForm.qty" type="number" min="1" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" required /></div></div><div :class="['text-xs p-3 rounded mt-2 border', transactionType === 'in' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-orange-50 text-orange-700 border-orange-200']"><i :class="transactionType === 'in' ? 'fa-solid fa-info-circle' : 'fa-solid fa-triangle-exclamation'" class="mr-1"></i> {{ transactionType === 'in' ? 'Jika Nama Barang & Kategori sama, stok akan bertambah.' : 'Pastikan stok cukup sebelum melakukan Barang Keluar.' }}</div><button type="submit" :class="['w-full py-2.5 rounded-lg font-medium transition mt-2', transactionType === 'in' ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-orange-600 text-white hover:bg-orange-700']">Simpan Data</button></form><div v-else class="space-y-4"><div v-if="transactionType==='out'" class="bg-orange-50 text-orange-800 text-xs p-3 rounded-lg border border-orange-200 space-y-1"><p class="font-bold"><i class="fa-solid fa-triangle-exclamation mr-1"></i>Perhatian Barang Keluar:</p><p>• Barang <b>harus sudah ada</b> di sistem, tidak akan auto-create.</p><p>• <b>Kunci pencocokan:</b> Kolom <b>shopName</b> + <b>itemName</b> harus <b>persis sama</b> (case-sensitive) dengan data di sistem.</p><p>• Jika salah ketik 1 huruf saja, stok tidak akan terkurangi & data ditolak.</p></div><div v-else class="bg-emerald-50 text-emerald-800 text-xs p-3 rounded-lg border border-emerald-200"><i class="fa-solid fa-info-circle mr-1"></i>Barang baru otomatis dibuat jika kombinasi <b>shopName</b> + <b>itemName</b> belum ada.</div><div class="text-xs border rounded-lg overflow-hidden"><div class="bg-gray-100 p-2 border-b font-bold text-gray-700"><i class="fa-solid fa-table-list mr-1"></i>Contoh Format Data:</div><table class="w-full text-left"><thead class="bg-gray-50 text-gray-600 uppercase text-[11px]"><tr><th class="p-2 border-b">date</th><th class="p-2 border-b">shopName</th><th class="p-2 border-b">itemName</th><th class="p-2 border-b text-center">qty</th></tr></thead><tbody><tr class="border-b"><td class="p-2">2025-01-15</td><td class="p-2">Toko Maju</td><td class="p-2">Parfum Mawar 50ml</td><td class="p-2 text-center font-bold">20</td></tr><tr><td class="p-2">2025-01-15</td><td class="p-2">Toko Jaya</td><td class="p-2">Sabun Cair Lavender</td><td class="p-2 text-center font-bold">50</td></tr></tbody></table></div><div @click="$refs.fileInput.click()" @dragover.prevent @drop.prevent="handleDrop" class="border-2 border-dashed rounded-xl p-8 text-center transition-colors cursor-pointer hover:border-blue-500 hover:bg-gray-50"><input type="file" ref="fileInput" class="hidden" accept=".xlsx, .xls" @change="handleFileSelect"><i v-if="!excelFile" class="fa-solid fa-cloud-arrow-up text-4xl text-blue-400 mb-3"></i><i v-else class="fa-solid fa-file-circle-check text-4xl text-green-600 mb-3"></i><p class="text-gray-700 font-medium">{{ excelFile ? excelFile.name : 'Drag & Drop file Excel di sini, atau Klik untuk pilih' }}</p><p class="text-xs text-gray-500 mt-1">Format: .xlsx / .xls</p></div><button type="button" @click="downloadTemplate" class="w-full py-2 border-2 border-dashed border-blue-300 text-blue-600 rounded-lg text-sm font-medium hover:bg-blue-50 transition"><i class="fa-solid fa-download mr-1"></i> Download Template Excel</button><div v-if="parsedExcelData.length > 0" class="text-xs border rounded-lg overflow-hidden"><div class="bg-green-50 p-2 border-b font-bold text-green-800">Preview ({{ parsedExcelData.length }} baris):</div><div class="max-h-40 overflow-y-auto"><table class="w-full text-left"><thead class="bg-green-100 text-green-800 uppercase text-[11px] sticky top-0"><tr><th class="p-2">date</th><th class="p-2">shopName</th><th class="p-2">itemName</th><th class="p-2 text-center">qty</th></tr></thead><tbody><tr v-for="(row, i) in parsedExcelData.slice(0, 10)" :key="i" :class="['border-b', row.qty <= 0 ? 'bg-red-50' : '']"><td class="p-2">{{ row.date || '-' }}</td><td class="p-2">{{ row.shopName || '-' }}</td><td class="p-2">{{ row.itemName || '-' }}</td><td class="p-2 text-center font-bold" :class="row.qty <= 0 ? 'text-red-600' : ''">{{ row.qty }}</td></tr></tbody></table></div><div v-if="parsedExcelData.some(r => r.qty <= 0)" class="p-2 bg-red-50 text-red-700 text-[11px] border-t font-semibold"><i class="fa-solid fa-circle-exclamation mr-1"></i>Ada baris dengan qty <= 0, baris tersebut akan dilewati/ditolak.</div></div><button type="button" @click="handleExcelUpload" :disabled="!excelFile || parsedExcelData.length === 0" :class="['w-full py-2.5 rounded-lg font-medium disabled:opacity-50 transition', transactionType === 'in' ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-orange-600 text-white hover:bg-orange-700']"><i class="fa-solid fa-upload mr-1"></i> Upload Excel</button></div></div></div></div></Transition></Teleport>
...
const filteredItems = computed(() => {
  ...
  if (startDate.value && endDate.value) { const s = new Date(startDate.value); s.setHours(0,0,0,0); const e = new Date(endDate.value); e.setHours(23,59,59,999); t = t.filter(i => { const d = new Date(i.createdAt); return d >= s && d <= e; }); }
...
const handleSingleUpload = () => {
  ...
  else { if (!user?.namaCabang) return showToast('Data cabang user tidak ditemukan', 'error'); presenter.uploadStock(transactionType.value, injectCabangToData([{ ...uploadForm }], user.namaCabang)); closeUploadModal(); }
};
...
const handleExcelUpload = () => {
  ...
  else { if (!user?.namaCabang) return showToast('Data cabang user tidak ditemukan', 'error'); presenter.uploadStock(transactionType.value, injectCabangToData([...parsedExcelData.value], user.namaCabang)); closeUploadModal(); }
};
...
const executeAdminUpload = () => {
  ...
  isAdminCabangModalOpen.value = false; closeUploadModal(); presenter.uploadStock(transactionType.value, finalData);
...
const processFile = async (file) => {
  ...
  try {
    const workbook = new ExcelJS.Workbook(); await workbook.xlsx.load(await file.arrayBuffer()); const data = [];
    workbook.getWorksheet(1).eachRow((row, rowNumber) => { if (rowNumber === 1) return; const values = row.values; values.shift(); data.push(values); });
    parsedExcelData.value = data.map(row => ({ date: row[0], shopName: row[1], itemName: row[2], qty: parseInt(row[3]) || 0 }));
  } catch (error) { showToast('Gagal memproses file Excel', 'error'); }
};

const downloadTemplate = async () => {
  const workbook = new ExcelJS.Workbook(); const sheet = workbook.addWorksheet('Template');
  sheet.columns = [{ header: 'date', key: 'date', width: 15 }, { header: 'shopName', key: 'shopName', width: 20 }, { header: 'itemName', key: 'itemName', width: 30 }, { header: 'qty', key: 'qty', width: 10 }];
  const hr = sheet.getRow(1); hr.font = { bold: true, color: { argb: 'FFFFFFFF' }, size: 11 }; hr.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF4F81BD' } };
  if (transactionType.value === 'in') { sheet.addRow({ date: '2025-01-15', shopName: 'Toko Maju', itemName: 'Parfum Mawar 50ml', qty: 20 }); sheet.addRow({ date: '2025-01-15', shopName: 'Toko Maju', itemName: 'Sabun Cair Lavender', qty: 50 }); }
  else { sheet.addRow({ date: '2025-01-20', shopName: 'Toko Maju', itemName: 'Parfum Mawar 50ml', qty: 5 }); sheet.addRow({ date: '2025-01-20', shopName: 'Toko Maju', itemName: 'Sabun Cair Lavender', qty: 10 }); }
  const buffer = await workbook.xlsx.writeBuffer(); const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
  const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `template_barang_${transactionType.value}.xlsx`; a.click(); URL.revokeObjectURL(url);
};

const handleDrop = (e) => { e.preventDefault(); if (e.dataTransfer.files.length) processFile(e.dataTransfer.files[0]); };
